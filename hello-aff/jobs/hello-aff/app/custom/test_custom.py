...
import pytest
from afftrainer import affTrainer
from affvalidator import affValidator
...
def get_cifar_trainer():
    with patch.object(affTrainer, "_save_local_model") as mock_save:
        with patch.object(affTrainer, "_load_local_model") as mock_load:
            yield affTrainer(train_task_name=TRAIN_TASK_NAME, epochs=1)


class TestaffTrainer:
    ...
    @patch.object(affTrainer, "_save_local_model")
    @patch.object(affTrainer, "_load_local_model")
    def test_execute_rounds(self, mock_save, mock_load):
        train_task_name = "train"
        trainer = affTrainer(train_task_name=train_task_name, epochs=2)
        # just take first batch
...
class TestaffValidator:
    def test_execute(self):
        validate_task_name = "validate"
        validator = affValidator(validate_task_name=validate_task_name)
        # just take first batch
